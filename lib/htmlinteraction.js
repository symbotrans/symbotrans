...
function buildPage() {
  ...
  // display last year of copyright range if there is a real range
  let y = new Date().getFullYear();
  if (y > 2019) {
    document.getElementById("copyright_year_field").innerHTML = "- " + y;
  }

  // display additional copyright authors
  ...
  if (additionalFooter != "") {
    document.getElementById("additional_footer_content").innerHTML = additionalFooter;
  }

  // hide some elements if only the translation result should be shown
  if (outputOnly) {
...
function parse_input() {
  ...
  let input = "";
  let input_length = input_field.value.length;
  let input_end = input_limit;

  let showLengthWarning = false;

  // if input contains "(...)" blocks
  // each block is translated as a whole
  // so input_limit has to regard the amount of those blocks and not the amount of characters

  if (bracketPartDetection && input_field.value.indexOf('(') !== -1 && input_field.value.indexOf(')') !== -1) {
    input_length = input_field.value.match(/\([^()]*\)/g).length;

    if (input_length > input_limit) {
      let pos = input_field.value.indexOf(')');
      for (let i = 1; i < input_limit && pos != 1; i++) {
        pos = input_field.value.indexOf(')', pos + 1);
      }
      input_end = pos + 1;
    }
  }

  // cut-off input if it exceeds input limitation
  if (input_length > input_limit) {
    input = input_field.value.substring(0, input_end);
  ...
  if (input.length === 0) {
    ...
    if (translatedParts.length === 0) {
      ...
      // show a warning above the translation result
      // if input exceeded input length limit
      if (showLengthWarning) {
        out = "<div class=\"warn\">At the moment the converter can only translate the first " + input_limit + " symbols!<br>You might want to split your input manually ...</div>"
      }
